...
export async function listUserFiles(userId, parentFolderId = null) {
  ...
  if (parentFolderId !== undefined && parentFolderId !== null) {
    ...
    // For root level, we need files with no parentFolderId or parentFolderId = null
    filterExpression += ' AND (attribute_not_exists(parentFolderId) OR parentFolderId = :nullValue)';
...
export async function moveFile(fileId, destinationFolderId, userId) {
  try {
    ...
    const command = new UpdateCommand({
      ...
      ExpressionAttributeValues: {
        ':parentFolderId': destinationFolderId || null, // If null, move to root
      },
      ReturnValues: 'ALL_NEW',
    });
